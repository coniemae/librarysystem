package com.conie.library.webcontroller.borrower;
...
@ManagedBean(name = "borrowerController")
@ViewScoped
	...
	public void update() {
		try {
			JsonClient.getSecureJsonPostResponse(BorrowerDto.class, UPDATE,
					borrower);
			setMsg(new FacesMessage(FacesMessage.SEVERITY_INFO, String.format(
					"Borrower %s is successfully updated.", borrower.getName()),
					borrower.getName()));
			initializeList();
			setAddedOrUpdated(true);
		} catch (GpsSatelliteException e) {
			LOGGER.error(e.getMessage(), e);
			setMsg(new FacesMessage(FacesMessage.SEVERITY_ERROR, String.format(
					"Failed to update %s!", borrower.getName()),
					borrower.getName()));
		}
	}	
